...
| `external` | ~19% | parser/Typed AST e graph preservam moves e handovers same-domain; consumo por parâmetro `external` de função `@extern(C)` é validado e mantém origem/destino no graph; ABI de handle, lifetime e lowering/runtime seguem pendentes e C11 continua fail-closed |
