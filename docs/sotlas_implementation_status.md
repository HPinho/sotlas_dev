...
| `device` | ~18% | parser/Typed AST e graph preservam owners, moves e handovers same-domain; leitura de campos no host é rejeitada; sincronização CPU/dispositivo, completion/reacquisition e runtime permanecem pendentes |
| `external` | ~20% | parser/Typed AST e graph preservam moves e handovers same-domain; consumo por parâmetro `external` de função `@extern(C)` é validado e mantém origem/destino no graph; leitura de campos no host é rejeitada; ABI de handle, lifetime e lowering/runtime seguem pendentes e C11 continua fail-closed |
