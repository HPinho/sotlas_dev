...
| `island` | ~98% | fronteiras funcionais, subset C11 por valor em parâmetros/retornos e campos `sole` com payload POD recursivo estão cobertos; borrows `whisper` call-scoped e handover `ISLAND→ISLAND` chegam ao graph/SIR; C11 executa handover com cleanup único; aliases armazenados, globals, enum/classes fora do subset e runtime de aliases ainda faltam |
