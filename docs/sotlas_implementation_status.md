...
- [ ] expansão do CFG estruturado além desse subset; produção agora baixa árvores aninhadas `if/else` cujas folhas são `return` void, além dos predicados booleanos e comparações inteiras já suportados; expressões condicionais como valores, corpos com efeitos arbitrários, joins com valores e integração geral de cleanup ainda faltam
...
| CFG + cleanup + defer para ownership | ~71% | além de retornos e loops com condições booleanas diretas, negadas, compostas `&&`/`||` ou comparação de parâmetros inteiros, árvores aninhadas `if/else` totalmente terminantes em `return` void geram CFG SIR/LLVM; marcadores de `share` e `quarantine` são colocados no bloco comum e nos ramos corretos, com ARC liberado antes de cada retorno; corpos arbitrários com efeitos, valores de retorno, operandos literais em comparações e todos os payloads de defer ainda não |
