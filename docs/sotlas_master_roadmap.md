...
- [ ] `region` como domínio de lifetime/ownership verificável; parser/Typed AST e graph preservam owners `sole` por valor, moves, merges invariantes, retornos e handovers same-domain; C11 executa análise canônica e testes nativos de transferência entre funções/bindings com drop único, rejeitando cruzamento implícito e use-after-move. Arena/lifetime graph e validação ampla de escapes ainda faltam;
