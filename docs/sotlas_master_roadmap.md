...
- [ ] `island` como subgrafo/região de ownership isolado — além de `quarantine`, o qualificador público `island T` é aceito somente para `sole` by-value, semeado como ISLAND/LIVE, integrado ao handover ISLAND→EXCLUSIVE e agora também a retornos com assinatura `-> island T`, que preservam ISLAND→ISLAND e rejeitam escape implícito para retorno EXCLUSIVE; aliases, demais fronteiras e runtime ainda faltam;
