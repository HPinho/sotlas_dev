...
- [ ] `region` como domínio de lifetime/ownership verificável; parser/Typed AST e graph preservam owners `sole` por valor, moves, merges invariantes, retornos, handovers same-domain e borrows call-scoped por `direct`/`whisper`; C11 executa análise canônica e testes nativos de transferência com drop único, incluindo cleanup recursivo de campos, arrays fixos e wrappers POD comuns contendo owners `region` `sole`, inclusive wrappers locais, em ordem reversa; `defer` vem antes do drop em early return e fallthrough (deinit detached do owner antes dos filhos); deinit de wrapper que acessa `self` com filhos region-owned falha fechado. Arena/lifetime graph e validação ampla de escapes ainda faltam;
- [ ] `device` como transferência de ownership CPU ↔ dispositivo com completion/reacquisition; parser/Typed AST e graph preservam o domínio e transferências same-domain; leitura de campos e borrows `direct`/`whisper` para código host são rejeitados, enquanto sincronização, completion/reacquisition e runtime de dispositivo continuam pendentes;
- [ ] `external` como ownership/lifetime atravessando FFI ou recurso externo; parser/Typed AST e graph preservam owners `sole` por valor, transferências same-domain e o consumo em função `@extern(C)` com parâmetro `external`; leitura de campos e borrows `direct`/`whisper` para código host são rejeitados; ABI de handle, lifetime e runtime continuam pendentes;
- [ ] `island` como subgrafo/região de ownership isolado — além de `quarantine`, `island T` é preservado nas fronteiras funcionais, locais e campos suportados; C11 cobre por valor parâmetros/retornos e campos `sole` com payload POD recursivo. Borrows `whisper` call-scoped de `island` com no-escape provado chegam ao graph/SIR/LLVM e executam em C11. Aliases armazenados, globals, enum/classes fora do subset e runtime de aliases continuam pendentes;
