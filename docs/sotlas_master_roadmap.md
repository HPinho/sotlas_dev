...
- [ ] `external` como ownership/lifetime atravessando FFI ou recurso externo; parser/Typed AST e graph preservam owners `sole` por valor, transferências same-domain e o consumo em função `@extern(C)` com parâmetro `external`; ABI de handle, lifetime e runtime continuam pendentes;
