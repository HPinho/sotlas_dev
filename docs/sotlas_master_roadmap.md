...
- [ ] `external` como ownership/lifetime em FFI e recursos externos; parser/Typed AST e graph preservam owners sole, transferencias same-domain e consumo FFI; C11 suporta somente owners sole repr(C) por valor via declaracao @extern(C) sem corpo quando cada owner recebido e consumido exatamente uma vez em todos os caminhos, validado por execucao nativa; campos, retornos, transferencias opacas, ABI geral de handles, lifetime e runtime seguem pendentes;
