...
- [ ] `island` como subgrafo/região de ownership isolado — além de `quarantine`, o qualificador público `island T` é aceito somente para `sole` by-value e agora é preservado em parâmetros, chamadas de função/método, retornos, locais, campos e enum payloads; destinos island exigem fonte já ISLAND/LIVE e registram ISLAND→ISLAND, enquanto EXCLUSIVE→ISLAND continua reservado a quarantine; handover ISLAND→EXCLUSIVE permanece a única saída explícita implementada; aliases, demais fronteiras e runtime ainda faltam;
