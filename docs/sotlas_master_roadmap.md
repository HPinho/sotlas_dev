...
- [ ] `external` como ownership/lifetime em FFI e recursos externos; parser/Typed AST e graph preservam owners sole, transferencias same-domain e consumo FFI; C11 suporta somente owners sole repr(C) por valor via declaracao @extern(C) sem corpo quando cada owner recebido e consumido exatamente uma vez em todos os caminhos, inclusive saidas de ramos exclusivos if/else, validado por execucao nativa positiva nos dois ramos e negativa para saida sem consumo; campos, retornos, transferencias opacas, ABI geral de handles, lifetime e runtime seguem pendentes;
...
- [ ] expandir CFG estruturado além desse subset protótipo; produção agora baixa árvores aninhadas `if/else` com folhas `return` void e condições booleanas/comparações inteiras já representáveis; expressões como valores, corpos arbitrários, integração geral de cleanup e demais terminadores ainda faltam;
