...
- [ ] `external` como ownership/lifetime em FFI e recursos externos; parser/Typed AST e graph preservam owners sole, transferencias same-domain e consumo FFI; C11 suporta owners sole `repr(C)` e wrappers `sole repr(C)` com campos `external` diretos e escalares POD por valor via declaracao @extern(C) sem corpo, exigindo consumo exatamente uma vez em todos os caminhos; testes nativos cobrem wrapper, ramos exclusivos e falha para saida sem consumo/layout com ponteiro; globals, classes/enums, arrays, retornos, transferencias opacas, ABI geral de handles, lifetime e runtime seguem pendentes;
