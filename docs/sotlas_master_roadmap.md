...
- [x] grafo backend-neutral de Ownership Domains registra owner, tipo, domínio, estado final e sink das transferências; transições cross-domain de quarantine/handover agora preservam source domain, target domain e domínio do destino sem inferência pelo backend.
...
- [x] placement ownership module-level unificado: domain transfers e cleanups ARC já posicionáveis (return/break/continue/backedge) são todos pré-validados antes da primeira mutação; `ShareInst`/`RetainInst` continuam apenas no plano até existir identidade source-stable de placement;
