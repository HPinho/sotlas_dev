...
""", encoding="utf-8")
    ...
    def test_region_handover_in_each_returning_branch_drops_once(self):
        source = """module app::region_branch_handover;
sole struct Token { value: u32; }
static mut destroy_count: u32 = 0;
fn Token_deinit(self: *mut Token) {
    unsafe { destroy_count = destroy_count + 1; }
...
fn main() -> i32 {
    let first: u32 = run(true);
    let second: u32 = run(false);
    if first == 41u32 && second == 41u32 && destroy_count == 4u32 {
        return 0;
    }
    return 1;
}
"""
        source_file = ROOT / "bootstrap" / "sotlas" / "test_region_branch_handover_temp.sotlas"
        executable = self.output_c.with_suffix(".exe")
        self.addCleanup(source_file.unlink, missing_ok=True)
        self.addCleanup(executable.unlink, missing_ok=True)
        source_file.write_text(source, encoding="utf-8")
...
fn discard(token: island Token) -> void { return; }
pub fn run() -> u32 {
    let source: island Token = Token { value: 41u32 };
    let destination: island Token = Token { value: 9u32 };
    discard(move destination);
    handover source to destination;
    return destination.value;
}
pub fn count_destroys() -> u32 { return destroy_count; }
"""
        driver_c = ROOT / "build" / "test_island_same_domain_main.c"
        driver_exe = ROOT / "build" / "test_island_same_domain.exe"
        self.addCleanup(driver_c.unlink, missing_ok=True)
        self.addCleanup(driver_exe.unlink, missing_ok=True)
        parsed = bootstrap.parse(source, filename="<island-same-domain-runtime>")
        bootstrap.check(parsed)
        self.output_c.write_text(bootstrap.emit_c(parsed), encoding="utf-8")
        driver_c.write_text("""
#include <stdint.h>
...
""", encoding="utf-8")
    ...
    def test_direct_island_method_receiver_runs_through_c11(self):
        source = """module app::direct_island_receiver_runtime;
sole struct Token {
    value: u32;
    fn inspect(self: direct Token) -> u32 { return self.value; }
}
...
"""
        driver_c = ROOT / "build" / "test_direct_island_receiver_main.c"
        driver_exe = ROOT / "build" / "test_direct_island_receiver.exe"
        self.addCleanup(driver_c.unlink, missing_ok=True)
        self.addCleanup(driver_exe.unlink, missing_ok=True)
        parsed = bootstrap.parse(
            source, filename="<direct-island-receiver-runtime>"
        )
        bootstrap.check(parsed)
        self.output_c.write_text(bootstrap.emit_c(parsed), encoding="utf-8")
        driver_c.write_text("""
...
uint32_t read_island(Token token);
int main(void) {
    return read_island((Token){81}) == 81 ? 0 : 1;
}
""", encoding="utf-8")
        compiler = _host_c_compiler()
        env = dict(os.environ)
